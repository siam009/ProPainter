...
def save_masks_to_disk(np_masks):
...
def save_frames_to_disk(frames):
    output_folder = str(uuid.uuid4())
    os.makedirs(output_folder, exist_ok=True)

    # Iterate through the frames and save each image
...
def get_frames_from_video(video_input, video_state):
    ...
    video_info = "Video Name: {},\nFPS: {},\nTotal Frames: {},\nImage Size:{} type:{}".format(video_state["video_name"],
                                                                                              round(video_state["fps"], 0),
                                                                                              len(frames), image_size, type(frames[0]).__name__)
    model.samcontroler.sam_controler.reset_image()
    model.samcontroler.sam_controler.set_image(video_state["origin_images"][0])

    # save_frames_to_disk(video_state["origin_images"])
...
def inpaint_video(video_state, resize_ratio_number, dilate_radius_number, raft_iter_number, subvideo_length_number, neighbor_length_number, ref_stride_number, mask_dropdown):
    ...
    # save_masks_to_disk(inpaint_masks)
...
iface.queue()
iface.launch(debug=True, server_name="0.0.0.0", server_port=8007)
